...
const LOAD_STEPS = [
  ...
  { msg: 'Finding Best Multi-Audio…',     delay: 3500  },
    ...
    try {
      ...
      for (const base of ADDONS) {
        try {
          ...
          // Pre-filter to ensure there is at least one non-mkv/non-hevc playable stream
      ...
      if (!json?.streams?.length) {
        throw new Error('All free stream providers are currently unavailable. Please try again later.')
      }
      ...
      // ──────────────────────────────────────────────────────────────────
      // SMART LANGUAGE SCORING: Find MULTI-AUDIO and HINDI/ENGLISH streams
      // ──────────────────────────────────────────────────────────────────
      const getScore = (s) => {
        let score = 0
        const url = (s.url || '').toLowerCase()
        ...
        else if (url.includes('.mp4')) score += 50

        // 2. Language Preference (High priority for Multi-Audio / Hindi / English)
        if (title.includes('multi') || title.includes('multi-audio') || title.includes('multi-subs')) score += 200
        if (title.includes('dual') || title.includes('dual-audio')) score += 150
        if (title.includes('hin') || title.includes('hindi')) score += 100
        if (title.includes('eng') || title.includes('english')) score += 50

        // 3. Quality preference
        if (title.includes('1080p')) score += 30
        if (title.includes('720p')) score += 20

        // 4. Penalize random foreign dubs if they don't explicitly mention English, Hindi, or Multi
        const hasEngOrMulti = title.includes('eng') || title.includes('multi') || title.includes('dual') || title.includes('hin')
        if (!hasEngOrMulti) {
            if (title.includes('ita') || title.includes('italian')) score -= 100
            if (title.includes('fre') || title.includes('french')) score -= 100
            if (title.includes('esp') || title.includes('spanish')) score -= 100
            if (title.includes('rus') || title.includes('russian')) score -= 100
        }
